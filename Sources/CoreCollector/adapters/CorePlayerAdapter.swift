...
open class CorePlayerAdapter: NSObject {
    ...
    public init(stateMachine: StateMachine) {
        ...
        NotificationCenter.default.addObserver(
            self,
            selector: #selector(willResignActive(notification:)),
    ...
    @objc
    func willResignActive(notification _: Notification) {
        stateMachine.videoStartFailureService.clearTimer()
    }

    @objc
    func willEnterForegroundNotification(notification _: Notification) {
        if !stateMachine.didStartPlayingVideo && stateMachine.didAttemptPlayingVideo {
